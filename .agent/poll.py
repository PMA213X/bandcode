...
def poll():
    ...
    try:
        ...
        if output:
            ...
            print(f"找到 {len(prs)} 个 PR Review 请求")
    ...
    print("")
    print("═══════════════════════════════════════════")
    print("  Coder Agent 循环报告")
    print("═══════════════════════════════════════════")
    print(f"  新任务数量: {new_tasks}")
    print(f"  已完成数量: {completed}")
    print(f"  当前状态: {'空闲' if new_tasks == 0 else '处理中'}")
    if details:
        print("  处理详情:")
        for d in details:
            print(f"  {d}")
...
def main():
    ...
    print(f"监听仓库: {REPO}")
    print(f"Agent 用户: {USERNAME}")
    
    while True:
        try:
            poll()
        except KeyboardInterrupt:
            print("收到中断信号，Agent 停止")
            break
        except Exception as e:
            print(f"主循环异常: {e}")
        
        print(f"等待 {POLL_INTERVAL // 60} 分钟后下一轮...")
