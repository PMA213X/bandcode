...
- **解决什么问题？** 熏掌门目前主要通过微信、抖音、小红书等渠道销售非遗熏鹅产品，用户咨询过程中大量问题高度重复（口味选择、储存方式、保质期、加热方式、物流时效等），人工客服重复回答效率低下，且无法保证回答一致性。
- **服务什么用户？** 熏掌门的潜在客户和老客户，通过微信、抖音、小红书等渠道咨询的用户，需要了解产品信息、下单、售后服务的用户。
- **为什么需要这个系统？** 实现 7x24 小时智能客服，降低人工客服重复劳动，通过知识库检索确保回答准确性，并支持用户意图识别和业务工具调用。
- **使用了哪些 AI 技术？** DeepSeek Chat 大语言模型、LangChain + LangGraph AI 框架、FAISS/Chroma 向量数据库 + Embedding Model 的 RAG 检索增强生成、基于 LLM 的意图识别、Function Calling 工具调用。
